...
func (g *ExcelGenerator) GenerateExcel(data *crEntity.OJKCompanyReportDataDto) ([]byte, error) {
	f := excelize.NewFile()
	sheet := "Company Report"
	...
	row = g.writeCompanyInfoSection(f, sheet, row, data, styles)
	row += 2
	...
	row = g.writeSideBySideSections(f, sheet, row, data, styles, data.Company.CompanyType)
	row += 2
...
func (g *ExcelGenerator) writeMainHeader(f *excelize.File, sheet string, startRow int, data *crEntity.OJKCompanyReportDataDto, styles *ExcelStyles) int {
	row := startRow
...
// writeCompanyInfoSection writes company details and recipient info
func (g *ExcelGenerator) writeCompanyInfoSection(f *excelize.File, sheet string, startRow int, data *crEntity.OJKCompanyReportDataDto, styles *ExcelStyles) int {
	row := startRow
	...
	companyInfo := []struct {
		...
		{"ID Perusahaan", data.Company.CompanyCode, false}, // This should be company ID
		{"Jumlah Peserta", fmt.Sprintf("%d orang", data.Summary.ParticipantCount), false},
	...
	recipientInfo := []string{
		fmt.Sprintf("Bapak/Ibu %s", data.Company.PicName),
		fmt.Sprintf("PIC PT %s", data.Company.FirstName),
		data.Company.Address,
		fmt.Sprintf("Di %s", data.Company.Domisili),
	}
...
func (g *ExcelGenerator) writeSideBySideSections(f *excelize.File, sheet string, startRow int, data *crEntity.OJKCompanyReportDataDto, styles *ExcelStyles, companyType string) int {
	row := startRow
	...
	// Add contribution data based on company type
	...
	if companyType == "DKP" {
		// For DKP company type, only show Employer Contribution
		iuranData = []struct {
			label string
			value float64
		}{
		...
		// For other company types, show all contribution types
		iuranData = []struct {
			label string
			value float64
		}{
